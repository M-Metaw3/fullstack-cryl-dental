...
const blogmodel= require('../models/blogsModule')
const fs = require('fs')
...
function extractLastTwoPathSegments(filePath) {
    const pathSegments = filePath.split('\\');
    const lastTwoSegments = pathSegments.slice(-2);
    return lastTwoSegments.join('\\');
  }
  
  const originalPath = 'G:\\New folder (4)\\fullstack-cryl-dental\\src\\images\\img\\image-1703426144748.png';
  const extractedPath = extractLastTwoPathSegments(originalPath);
  console.log(extractedPath);
extractLastTwoPathSegments(image.path)
